...
\begin{document}
    ...
    \begin{center}
        \boxed{ \mathbf{d \sigma = m_1 m_2 \frac{(2 \pi)^4 |M_{fi}|^2 \delta^4 (P_f - P_i)}{[(p_1 \cdot p_2)^2 - m_1^2 m_2^2]^{1/2}} \prod_{n = 1}^{N_f} \frac{m_n d^3 \vec{p}_n}{(2 \pi)^3 2 E_n} } }
    \end{center}
    ...
    \begin{center}
        \begin{tikzpicture}
            % Horizontal Line
            \draw[-, thick] (-3, 0) -- (-0.1, 0) node[midway, above] {\small};
            \draw[-, thick] (3, 0) -- (0.1, 0) node[midway, above] {\small};
    
            % Arrow after Collision
            \draw[->, thick] (-0.1, 0) -- (-2, 2) node[midway, left] {\small};
            \draw[->, thick] (0.1, 0) -- (2, -2) node[midway, right] {\small};
    
            % Theta
            \node at (0.8, -0.3) {\small $\theta$};
